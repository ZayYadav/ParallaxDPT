...
PARALLAX_ENCRYPT void patchClass(__unused const char* descriptor,
    ...
    if(descriptor != nullptr && UNLIKELY(parallax_strstr(descriptor, junkClassName) != nullptr)) {
        ...
        if(isdigit(static_cast<unsigned char>(ch))) {
            ...
            if (prctl(PR_GET_NAME, thread_name) == 0
                && parallax_strstr(thread_name, AY_OBFUSCATE("Verification")) != nullptr) {
...
PARALLAX_ENCRYPT void* fake_mmap(void* __addr, size_t __size, int __prot, int __flags, int __fd, off_t __offset){
    ...
    char fd_path[256] = {0};
    parallax_readlink(__fd,fd_path, ARRAY_LENGTH(fd_path));
...
PARALLAX_ENCRYPT ssize_t fake_write(int fd, const void *const buf, size_t count) {
    ...
    if(buf != nullptr && count > 0x70) {
        ...
        if (UNLIKELY(parallax_memcmp(buf, dex_magic, 4) == 0)) {

            std::string hex = to_hex((uint8_t *) buf + 9, 20);
            DLOGD("dex sign: %s", hex.c_str());
            if (parallax_strncasecmp(hex.c_str(), g_shell_config.dex_sign.c_str(), 40) == 0) {
                parallax_crash();
            }
        }
    }
