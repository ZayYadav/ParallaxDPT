...
size_t parallax::DexFileUtils::readUleb128(uint8_t const * const data, uint64_t * const val) {
    uint64_t result = 0;
    size_t read = 0;
    for(int i = 0;i < 5;i++){
        uint8_t b = *(data + i);
...
size_t parallax::DexFileUtils::readFields(uint8_t *data, parallax::dex::ClassDataField *fields, uint64_t count) {
    size_t read = 0;
...
size_t parallax::DexFileUtils::getFieldsSize(uint8_t *data, uint64_t count) {
    size_t read = 0;
    for (uint64_t i = 0; i < count; ++i) {
        uint64_t fieldIndex = 0;
        read += readUleb128(data + read, &fieldIndex);
...
size_t parallax::DexFileUtils::readMethods(uint8_t *data, parallax::dex::ClassDataMethod *method, uint64_t count){
    size_t read = 0;
