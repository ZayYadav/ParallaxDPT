...
parallax代码分为两个部分，一个是processor，另一个是shell。
...
Hook函数时机最好要早点，parallax在`.init_array`节被加载时开始进行一系列Hook

```cpp
__attribute__ ((constructor)) void init_parallax() {
    parallax_hook();
}
```
...
答案是肯定的，parallax之前就是使用的LoadMethod函数作为Hook点，在LoadMethod函数里面做CodeItem填充操作。

但是后来发现，LoadMethod函数参数不太固定，随着Android版本的升级可能要不断适配，而且每个函数都要填充，会影响一定的性能。

### (2) 加载dex

所有apk中的dex在处理阶段parallax都把它们放到了单独的zip文件中，不存在apk中了，所以App启动时要手动加载。
...
具体可以看[ProxyComponentFactory](https://github.com/ZayYadav/ParallaxDPT/blob/main/ParallaxShell/shell/src/main/java/com/parallax/shell/ProxyComponentFactory.java)类，这里不再贴出。
